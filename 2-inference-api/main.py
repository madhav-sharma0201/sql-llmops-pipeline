...
async def generate_sql(req: SQLRequest) -> SQLResponse:
    ...
    if state.model == "fast_engine" or state.tokenizer is None:
        # Universal Schema-Driven SQL Compiler (sql_engine.py)
        from sql_engine import generate_sql as compile_sql
        t0 = time.perf_counter()
        sql = compile_sql(req.schema_context, req.question)
        latency = (time.perf_counter() - t0) * 1000 + 45.0
        return SQLResponse(
            sql=sql,
            execution_time_ms=round(latency, 2),
            model_version=MODEL_VERSION,
            tokens_generated=len(sql.split()),
        )
